#include <iostream>
using namespace std;

//////////////////////////////////////////
using bit32 = unsigned;
using bit8 = unsigned char;

// S盒
const bit8 Sbox[256] = {
    };

// 常数FK
const bit32 FK[4] = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc};

// 常数CK
const bit32 CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279};

// 循环左移
#define lift32(val,n)(val >> (32 - n)) + (val << n)

// T 置换
bit32 deal_T(bit32 input)
{
    bit32 ret = 0;
    bit8 *point = (bit8 *)&input;
    bit8 a[4] = {point[3], point[2], point[1], point[0]};
    for (int i = 0; i < 4; i++)
    {
        ret = ret << 8;
        ret += Sbox[(a[i] / 16) * 16 + (a[i] % 16)];
    }
    ret = ret ^ (lift32(ret, 2)) ^ lift32(ret, 10) ^ lift32(ret, 18) ^ lift32(ret, 24);
    return ret;
}

// T’置换
bit32 deal_point_T(bit32 input)
{
    bit32 ret = 0;
    bit8 *point = (bit8 *)&input;
    bit8 a[4] = {point[3], point[2], point[1], point[0]};
    for (int i = 0; i < 4; i++)
    {
        ret = ret << 8;
        ret += Sbox[(a[i] / 16) * 16 + (a[i] % 16)];
    }
    ret = ret ^ (lift32(ret, 13)) ^ lift32(ret, 23);
    return ret;
}

// 密钥生成
void key_generate(bit32 MK[4], bit32 rK[32])
{
    bit32 K[36] = {MK[0] ^ FK[0], MK[1] ^ FK[1], MK[2] ^ FK[2], MK[3] ^ FK[3]};
    for (size_t i = 0; i < 32; i++)
        K[i + 4] = rK[i] = K[i] ^ deal_point_T(K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i]);
}
/////////////////////////////////////////

void SM4(bit32 MK[4], bit32 input[4])
{
    bit32 rK[32];
    bit32 X[36] = {input[0], input[1], input[2], input[3]};
    key_generate(MK, rK);
    for (int i = 0; i < 32; i++)
        X[i + 4] = X[i] ^ deal_T(X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rK[i]);
    input[0] = X[35];
    input[1] = X[34];
    input[2] = X[33];
    input[3] = X[32];
}

int main()
{
    bit32 input[4] = {0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210};
    bit32 key[4] = {0x01234567, 0x89ABCDEF, 0xFEDCBA98, 0x76543210};
    SM4(key, input);
    for (int i = 0; i < 4; i++)
        cout << hex << "0x" << input[i] << "  ";
    cout << endl;
    return 0;
}